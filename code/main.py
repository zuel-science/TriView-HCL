...
from module import TriView_HCL
import warnings
import datetime
...
def train():
    ...
    model = TriView_HCL(args.hidden_dim, 
                 feats_dim_list, 
                 args.feat_drop, 
                 args.attn_drop,
                 P, 
    ...
    for epoch in range(args.nb_epochs):
        ...
        if loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), 'TriView_HCL-main/TriView_HCL_'+own_str+'.pkl')
        else:
            cnt_wait += 1
    ...
    model.load_state_dict(torch.load('TriView_HCL-main/TriView_HCL_'+own_str+'.pkl'))
    model.eval()
    os.remove('TriView_HCL-main/TriView_HCL_'+own_str+'.pkl')
